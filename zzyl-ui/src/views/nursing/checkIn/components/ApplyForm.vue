...
import { parseIdCard, validateIdCard } from '@/utils/validate';
// 选择老人列表弹层
...
//身份证号变化时，自动解析出生日期、年龄和性别
const handleIdCardChange = () => {
  if (!formData.value.idCardNo || formData.value.idCardNo.length !== 18) {
    return;
  }
  
  const parsedInfo = parseIdCard(formData.value.idCardNo);
...
// 监听身份证号变化，无论是手动输入还是弹窗选择都能触发解析
watch(
  () => formData.value.idCardNo,
  (newIdCard) => {
    if (newIdCard && newIdCard.length === 18) {
      const parsedInfo = parseIdCard(newIdCard);
      if (parsedInfo && parsedInfo.isValid) {
        // 只有当弹窗选择时才覆盖已有数据，手动输入时不覆盖用户可能手动修改的数据
        ...
        if (formData.value.sex === undefined || formData.value.sex === null) {
          formData.value.sex = parsedInfo.sex;
        }
      }
    }
